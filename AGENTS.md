...
- **隐私与本地环境信息（强制，2026-09-13 加强）**：**源码、测试与 fixture、内部文档（开发日志 / 审查 / QA / 设计说明）、工单（`.beads/issues.jsonl` 及其评论）、脚本、CI 配置、截图与证据说明** —— 一律不得残留任何本机数据或敏感数据：本机绝对路径（盘符路径、`/Users/<name>`、`C:\Users\<name>`、UNC 主机名与共享名）、机器名与用户名、个人目录或资源库名称、真实库内文件名与内容、隐私数据、访问令牌 / API Key / 密码 / Cookie / 凭据。
  ① **举例同样受限**：为说明问题而引用真实路径或真实资源库名，等同于再一次泄露 —— 一律改用类别描述或明显虚构的示例。（2026-09-13 实测教训：更正类工单里为举例引用真实串并推送，等于把刚清掉的东西又写回去。）
  ② **替换写法**：用可读的大白话描述替代真实路径（如「本地磁盘上的工作副本」「隔离 fixture 目录」「用户下载目录」），不要统一塞成 `<路径>` 之类的占位符，否则记录失去可读性。
  ③ **测试与 fixture**：只用临时目录、相对路径或脱敏 fixture；夹具里合法的假路径（`/Users/example/...`、`C:\a.png` 等）必须进入扫描白名单并逐条写明「为什么这条是假的」，**禁止用放宽正则代替白名单**（否则门禁被噪声淹没而形同废弃）。
  ④ **提交前验证**：对被跟踪的全仓库扫描盘符路径、家目录 / 用户目录、UNC 主机共享、已知资源库名与用户名、凭据类口令。扫描门禁、口令清单与白名单见 `Serpent-b78285`（P0：提交文件全面脱敏）。
  ⑤ **历史不可回收，所以在源头禁止**：前进式脱敏只清当前树，历史仍可被 `git log -S`、`git show <旧提交>`、`blame` 查到；要真正清除必须重写历史（force-push 所有 ref + 重打 tag），而 fork、托管端悬空对象、CDN 缓存与已公开内容都无法撤回。不要依赖事后清理。
  ⑥ **审查固定项**：任何提交在 review 时必须核对本条（**包括工单与文档里的示例**）；发现残留立即更正，并在对应工单记录残留范围与是否重写历史。
...
5. **禁止使用 `git worktree`（2026-09-15 用户要求，强制）**：本项目不使用 `git worktree`——既不拿它做多轨道并行开发，也不拿它建对照基线。原因：① 多工作树只能靠 junction/symlink 共享 `node_modules`/`media-binaries`，而删除工作树时会**穿过 junction** 递归删除目标内容——实测（`Serpent-9b1af8`）把主工作树的 `node_modules` 清空，native 模块与 Electron 二进制一并丢失且不进回收站；② 多工作树让依赖版本、构建产物（`.vite`/`out`）与测试证据难以对账，等于把环境问题伪装成代码问题。需要干净基线做对照实验时用 `git stash`（跑完 `git stash pop`），或把对照产物复制到仓库外目录；需要并行推进时用分支 + 串行交接，不要多检出。已经存在的检出应清理，不要在其上继续开发。恢复被破坏的依赖后，必须实测 better-sqlite3 双 ABI、FTS5 probe 与至少一条真实 Electron E2E 才算恢复。
