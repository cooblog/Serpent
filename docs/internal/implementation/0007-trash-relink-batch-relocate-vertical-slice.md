...
- 跨资源库回收站或联合删除。
- 回收站空间配额或逐项大小预估。
- 链接资产的 Serpent 回收站。**2026-09-15 用户决定（修订本条）**：链接资产与链接文件夹都**没有**「移入回收站」；链接条目不属于资源库，进 Serpent 回收站也无法恢复源文件。链接条目的动作收敛为两种——「移除链接文件夹/记录」（只删链接记录，源文件一律不动）与「强制从硬盘删除」（永久删除源文件，且**不再使用系统回收站**）。旧实现把每个文件送进系统回收站，实测 126 ms/文件、1.5 万文件约 31 分钟且期间独占调度器，已移除；参见工单 `Serpent-7edf6e`。
