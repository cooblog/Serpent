...
- ~~三条轨道在独立 git worktree 并行实施，主工作树只由主 agent 做合流；冲突按 T5 → T7 → T6 顺序合并。~~ **已作废（2026-09-15 用户要求：本项目禁止使用 `git worktree`）**：本条只作当时的历史记录，后续按分支 + 串行交接推进；原因见 `AGENTS.md` §磁盘与工作区洁净纪律 第 5 条与 `Serpent-9b1af8`。
