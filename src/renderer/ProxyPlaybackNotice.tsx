...
export function shouldShowProxyPlaybackNotice(input: {
  playbackMode: string | undefined;
  dismissed: boolean;
}): boolean {
  return input.playbackMode === "proxy" && !input.dismissed;
}

/** Low-presence explanation for a viewer that had to fall back to a proxy. */
export function ProxyPlaybackNotice({ onHide }: ProxyPlaybackNoticeProps) {
  const t = useT();
  return (
